...
describe('Issues router', () => {
    ...
    describe('POST /api/issues', () => {
        it('returns 201 and issue with valid token and issue', async () => {
          const token = await registerToken();
          const response = await request(server).post("/api/issues").send({title: "Test title", category: "roads"}).set("authorization", token)
          expect(response.status).toBe(201);
        });
        it('returns 401 if invalid token is sent', async () => {
          const response = await request(server).post("/api/issues").send({title: "Test title", category: "roads"}).set("authorization", 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImVsZWFzYWhAbGFtYmRhLmNvbSIsInVzZXJpZCI6NSwiaWF0IjoxNTY5NTE5MjIyLCJleHAiOjE1Njk5NTEyMjJ9.SGZMwP_w7SLzwwVPXNeLm4am6qZ1sxOgx1RWtv0eLB0')
          expect(response.status).toBe(401);
        });
        it('returns 500 if category is not included on issue', async () => {
          const token = await registerToken();
          const response = await request(server).post("/api/issues").send({title: "Test title"}).set("authorization", token)
